...
- `IncomingDir` 写入可持久化 Settings 子集。`OverlayPersistableSettings` / `InitClient` 对空 `IncomingDir`、`WebCacheDir` 不覆盖 LoadState 已恢复的非空值。`BuildSettings` 仍只映射 Config 已有字段，不发明 Incoming 布局。
...
- 兼容路线图收口（更正 #36）：MultiPacket EXT2 出站/宣告与搜索括号/引号已核对不做；UDPVer 2+、macOS `F_PREALLOCATE` 为后续；IncomingDir 已写入可持久化子集且 Overlay 空路径不覆盖恢复值，Config 字段需用户决策。分片大小沿用 eMule 习惯 9.28 MB / 9728000。
