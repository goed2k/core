...
func (m tuiModel) renderEmptyState(width int) string {
	lines := []string{
		...
		fmt.Sprintf("kad=%t  kadv6=%t  upnp=%t  crypt=%t  secident=%t", m.cfg.enableKAD, m.cfg.enableKADV6, m.cfg.enableUPnP, m.cfg.enableCryptLayer, m.cfg.enableSecIdent),
		fmt.Sprintf("identity-key=%s", emptyFallback(m.cfg.identityKeyPath, "-")),
		fmt.Sprintf("nodes.dat=%s", emptyFallback(m.cfg.kadNodesDat, "-")),
